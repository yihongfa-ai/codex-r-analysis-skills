...
7. Reproducibility and validation.
   ...
   - Create `<project>_dispatch.ipynb` under `notebooks/projects/<project>/` as the main notebook entrypoint. It may call the project R scripts, including `R/scripts/<project>/00_master.R`, as backend orchestration code.
   - Create `<project>_complete_reproducible.ipynb` under `notebooks/projects/<project>/` as a standalone, analysis-only backup. This notebook is the substantive reproducibility artifact, not a second dispatcher or an index of scripts. It must contain all code needed to reproduce the data-analysis results without calling other notebooks or external R scripts. It must be split into many focused cells, with clear section headings, detailed functional introductions explaining what each part does, detailed code comments, visible outputs from every code cell, and interpretation/explanation below each major result. It must not include formal writing, Word generation, QMD rendering, `officer`/`flextable` document assembly, or `source()` calls.
   ...
   - Generate the default analysis QMD from `<project>_complete_reproducible.ipynb`, not from `<project>_dispatch.ipynb`. The complete notebook contains explanations for human review; the QMD must keep semantic functional section headings from the complete notebook when they identify retained analysis code, and must omit explanation-only Markdown, functional-introduction paragraphs, result-interpretation Markdown, and interpretation-only code cells/outputs such as `cat("解释...")` or comments headed `# 解释`. Retained QMD code chunks must match the retained analysis code cells line-for-line, in the same order, and QMD outputs should preserve retained analysis outputs as static output blocks/images. Do not add or copy result-interpretation prose below copied outputs.
   - Run `Rscript --vanilla` on the master script and execute both notebooks when possible.
...
- Reproducible R scripts, Quarto/QMD source, Jupyter notebooks, cleaned/analytic data, logs, and session information; for long-running reusable projects, optional tested R helper functions or package-style structure.
- Notebook execution artifacts:
  - `<project>_dispatch.ipynb`: the canonical notebook entrypoint for the full formal run, executed with the Anaconda/base Jupyter `ir` R kernel. It may use `R/scripts/<project>/00_master.R` and related scripts as backend orchestration.
  - `<project>_complete_reproducible.ipynb`: a standalone, analysis-only R-kernel notebook backup. It must be self-contained for data analysis, split into focused cells, and persist outputs in the notebook file. It must include the full analysis code directly in notebook cells, organize the analysis into titled sections, introduce each section's function before the code, use detailed comments, show results, and explain major results below the outputs. A notebook that only calls `00_master.R`, `source()`s scripts, or lists files is only a dispatcher and is not acceptable as the complete reproducibility notebook.
- QMD execution artifact:
  - `<project>_results.qmd` or `<project>_complete_reproducible.qmd` must be derived from `<project>_complete_reproducible.ipynb`, not from `<project>_dispatch.ipynb`. Markdown may be adjusted for Quarto readability, and QMD must keep semantic functional section headings from the complete notebook when they identify retained analysis code, but it must omit explanation-only Markdown, functional-introduction paragraphs, result-interpretation Markdown, and interpretation-only code cells/outputs such as `cat("解释...")`, comments headed `# 解释`, or cells whose purpose is only to explain a prior result. For retained analysis cells, code chunks must be an exact line-for-line copy of the corresponding notebook code cells, in the same order. Do not insert chunk-local `#|` options into these code chunks if that would make the code differ from the notebook; use YAML-level execution options instead. The QMD should preserve retained analysis outputs as static output blocks/images so the QMD output view matches the notebook's analysis output view. After generating QMD, run a code-and-output equivalence audit by extracting retained QMD R chunks and copied output blocks/images and comparing them to the retained complete-notebook analysis code cells and saved analysis outputs. Report omitted explanation cell/output counts and confirm that QMD headings are functional rather than mechanical.
...
- Treat `<project>_complete_reproducible.ipynb` as the source of truth for formal data-analysis code, but keep its human-facing explanations out of QMD. The analysis QMD must preserve retained analysis code cells, in the same order, with identical code text, after excluding explanation-only cells.
...
- QMD section headings must be semantic and functional, inherited from the nearest relevant heading in `<project>_complete_reproducible.ipynb`, such as `Descriptive Statistics`, `Model Estimation`, `Interaction Figures`, or `Hypothesis Summary`. Never generate headings such as `Code Cell 1`, `Code Cell 2`, `Chunk 1`, or other ordinal/mechanical labels. If a retained code cell has no preceding functional heading, first add a functional heading to the complete notebook or derive a concise heading from the cell purpose before writing the QMD. If several retained chunks belong to one section, keep the functional heading once or use meaningful functional subheadings, not numbered code-cell headings.
- A valid QMD-generation run must report: retained notebook analysis-code-cell count, omitted explanation-cell count, QMD R-chunk count, retained output-object count, omitted explanation-output count, and whether every extracted QMD code chunk and copied output object matches the corresponding retained complete-notebook analysis code cell/output object.
- If the QMD is intended as a formal manuscript file rather than an analysis reproducibility file, use a distinct filename such as `<project>_manuscript_results.qmd`; do not let that manuscript QMD replace the code-equivalent analysis QMD.
